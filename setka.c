...
void Setka (int *st, double *X, double *Y, int *bot, int *top, P_she *p_s)
{
  ...
  set_str (st, X, Y, bot, top, p_s,
           &i, 7, 3, 7, n,
           2 * COEF, 0, n, n/*bot[i]<0*/);

  for (j = 1; j < m - 2; j++)
    {
      set_str (st, X, Y, bot, top, p_s,
               &i, 1, 0, 2, n,
               2 * COEF, j * p_s->h_y, n , n);
    }
  set_str (st, X, Y, bot, top, p_s,
           &i, 1, 0, 2, n,
           2 * COEF, j * p_s->h_y, 3 * n - 2 , n);

  set_str (st, X, Y, bot, top, p_s,
           &i, 7, 5, 5, 2 * n - 2,
           0, 1 * COEF, 3 * n - 2 , (3 * n - 2) * (m + 1) /*bot[i] < 0*/);

  set_str (st, X, Y, bot, top, p_s,
           &i, 7, 0, 2, n,
           2 * COEF, 1 * COEF, 3 * n - 2 , 3 * n - 2);
  ...
  for (j = 1; j < 2 * m - 2; j++)
    {
      set_str (st, X, Y, bot, top, p_s,
               &i, 4, 0, 2, 3 * n - 2,
               0, 1 * COEF + j * p_s->h_y, 3 * n - 2 , 3 * n - 2);
    }
